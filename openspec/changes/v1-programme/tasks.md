...
- [ ] 1.2 Build change `model-foundation-and-record-full` from `data-and-privacy` (the model rules) and `record`: the sixteen neutral models with kinds, entry versions and the on-read winner, per-row change moments and deleted flags, keys not relationships, settings rows, the Reconciler as a pure function, the frozen-names file; then the Today stack, Where, Context, edit, delete, "didn't record", "Pause for today", "Fasting today", earlier days, collapse, gap bands and their switch, "Day starts at".
...
- [ ] 2.1 Build change `programme-engine` from `programme`: `ProgrammeConstants` as a value, the stage state as a pure function over value facts with stored openings, week counting from stage 2, the Programme screen model, opening cards in the card slot, card answer rows.
...
- [ ] 2.4 Build change `reminders` from `reminders`: the scheduler over the rolling horizon, every type including "Worksheet review" and "Check-in", actions from userInfo, the action queue, snooze state, the cap with planned meals separate, quiet hours with wrap-around, the Time Sensitive entitlement.
...
- [ ] 3.2 Build change `weekly-review` from `weekly-review` and the re-screening and deterioration rules in `safeguarding`: the summary with frozen counts, the two-step self-harm item, reflection, the pinned note, taking stock and its module completion.
...
- [ ] 3.6 Build change `staying-on-track` from `staying-on-track`: the maintenance plan, the finish, reduced cadence through `finishDate`, check-ins as a reminder type and a Today line, restart with the re-screen rule.

## 4. Data and release

The first TestFlight cut is tasks 1.1 to 1.5, 2.1 to 2.4, 3.2, 4.1, 4.2 and 4.3. It leaves out the requirements that `deferred.md` lists and labels each one as deferred. Tasks 2.5, 3.1, 3.3 to 3.6 and 4.1b follow it.

- [ ] 4.1 Build change `local-delete-all` from `data-and-privacy`: Delete-all of the whole directory, the deleted screen, the launch marker and safe mode, the MetricKit count, Diagnostics. This is in the first cut.
