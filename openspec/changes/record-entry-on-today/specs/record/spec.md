...
A record day starts at 04:00 and ends at 03:59 the next calendar day. The app MUST compute the record day in the device's current time zone. The app MUST set the day start with the calendar's "04:00 on this calendar date" rule. A record day on a clock-change date is then 23 or 25 hours long. The current record day is the record day that contains the current moment. An entry is in a record day when its time is in that day's interval.

#### Scenario: Entry after midnight
- **WHEN** an entry has the time 00:30 on Friday 25 September
- **THEN** the app assigns it to the record day of Thursday 24 September
...
The app MUST let the person create an entry with a time, a What text and a "felt like a binge" star. The time MUST default to the moment the new-entry screen opens. The app MUST accept any entry time from the start of the previous record day to the moment the person saves. The time control MUST let the person choose the date and the time within that range. The app MUST store the time to the minute. The What field MUST accept free text and MUST accept an empty text. The app MUST remove white space and line breaks from the start and end of What and keep the rest as typed. The app MUST NOT offer a food database, portions or calories. The app MUST NOT show a placeholder in What.
...
When the person saves, the app MUST close the new-entry screen with the system's standard dismissal and show Today. Today MUST scroll so that the saved entry is on screen. The app MUST NOT add a confirmation, a message, a colour change, an icon, a sound, a haptic or an animation on save. The system's standard sheet dismissal and standard list update are not additions. When the person turns the star on or off, the app MUST NOT show a confirmation, a message, an icon or a colour change elsewhere. The app MUST NOT add a sound or a haptic. The star control's own state change and the system toggle's standard haptic are not additions.
...
Today MUST show every entry in the current record day. When the previous record day has at least one entry, Today MUST show those entries above, under that day's date heading. Today MUST NOT show a heading for a previous record day with no entries. Today MUST order the entries in each day by entry time, earliest first. When two entries have the same time, Today MUST order them by creation moment, earliest first. Today MUST show each entry's time. Today MUST show each entry's What when it is not empty. Today MUST show a control that opens the new-entry screen in one tap.
...
Today MUST compute the current record day again when the app becomes active. Today MUST compute it again at 04:00 while Today is on screen.
...
Today's heading MUST show the record day's weekday and date. Between 00:00 and 03:59 the current day's heading MUST add ", night" after the date. Today MUST show a starred entry with an asterisk glyph in the same colour, weight and size as the entry's time. The glyph MUST use no accent colour and no fill. A starred row MUST have the same background, height, spacing and text style as an unstarred row. Every row MUST have the same height and spacing, whatever the time since the previous entry. Today MUST show absolute times only, with no relative times, no dividers and no grouping within a record day. Today MUST NOT show a count of entries, a total, a streak, a score or any text about the absence of entries. On an empty current record day, Today MUST show only the heading and the control that opens the new-entry screen. The visible strings of this change are "Today", "What", "felt like a binge", "Save" and "Cancel". The new-entry screen MUST have no title. The app MUST NOT use the words meal, food, log, diary, intake or eat in any string. A reviewer checks these rules on the simulator. No test in this change covers them.
...
- **THEN** Today shows the heading and the control that opens the new-entry screen, and nothing else
...
Each row on Today MUST be one accessibility element. The row's label MUST hold the time, then the What when not empty, then "felt like a binge" when the star is on. A comma and a space MUST separate the parts. Every control on Today and the new-entry screen MUST have a VoiceOver label. The star control's visible label and VoiceOver label MUST both be "felt like a binge". The star MUST NOT depend on colour alone. Text on Today and the new-entry screen MUST use system text styles and MUST scale with Dynamic Type. The app MUST NOT limit What to one line on Today.
...
- **THEN** the change's README records who walked the starred-entry path, on which date, and their answer for each screen
