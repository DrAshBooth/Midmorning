...
struct AppLockRootView: View {
    ...
    init(store: RecordStore) {
        self.store = store
        let enabledSetting = store.localSettingValue(forKey: AppLockSettingsKeys.enabled)
        ...
        let faceOrTouchOnly = store.localSettingValue(forKey: AppLockSettingsKeys.faceOrTouchOnly) == "true"
        let lockAfterSeconds = store.localSettingValue(forKey: AppLockSettingsKeys.lockAfterSeconds)
    ...
    private func checkEnrolmentStateIfNeeded() {
        ...
        let kept = store.localSettingValue(forKey: AppLockSettingsKeys.enrolmentStateHash)
        if kept == nil {
            try? store.setLocalSetting(key: AppLockSettingsKeys.enrolmentStateHash, value: current)
            return
        }
