...
final class ReviewSaveStoreTests: XCTestCase {
    ...
    /// The moment every read runs at: after every review this file writes,
    /// so the store's future-dated rule (mm-t12.33) ignores none of them.
    private var readNow: Date { at(2030, 1, 1) }
    ...
    private func pinnedNote(_ store: RecordStore) throws -> String? {
        let latest = try store.reviewRowWinners(kind: .weeklyReview, now: readNow)
    ...
    private func isFinished(_ store: RecordStore, week: Int) throws -> Bool {
        guard let row = try store.review(kind: .weeklyReview, dueDateKey: dueDayKey(week), now: readNow) else { return false }
    ...
    private func opensWithDeteriorationPage(_ store: RecordStore, week: Int, at moment: Date) throws -> Bool {
        ...
        for checkedWeek in (week - needed + 1)...week {
            guard let row = try store.review(kind: .weeklyReview, dueDateKey: dueDayKey(checkedWeek), now: readNow),
                  let starred = ReviewAnswersPayload.decode(row.answersJSON).frozenCounts?.starred else { return false }
            counts.append(starred)
        }
        guard let current = try store.review(kind: .weeklyReview, dueDateKey: dueDayKey(week), now: readNow),
    ...
    func testReopenThenDoneKeepsSelfHarmAnswered() throws {
        ...
        XCTAssertEqual(try store.review(kind: .weeklyReview, dueDateKey: dueDayKey(2), now: readNow)?.selfHarmAnswered, true, "the app does not ask the item again")
        try save(.answersSoFar, store, week: 2, selfHarmStepOneAnswered: false, at: at(2026, 10, 14, 10))
        XCTAssertEqual(try store.review(kind: .weeklyReview, dueDateKey: dueDayKey(2), now: readNow)?.selfHarmAnswered, true)
    }
    ...
    func testTheDeteriorationPageShowsOncePerReview() throws {
        ...
        let row = try store.review(kind: .weeklyReview, dueDateKey: dueDayKey(5), now: readNow)
    ...
    func testTheReviewsListShowsBothRunsAfterARestart() throws {
        ...
        let winners = try store.reviewRowWinners(kind: .weeklyReview, now: readNow)
