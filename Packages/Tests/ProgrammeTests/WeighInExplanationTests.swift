...
final class WeighInExplanationTests: XCTestCase {
    ...
    func testExplanationInKilograms() {
        XCTAssertEqual(WeighInExplanation.text(unit: .kg), "Weekly swings of a kilo or two are normal and mean nothing on their own.")
    }

    /// Scenario: Explanation in stone and pounds.
    func testExplanationInStoneAndPounds() {
        XCTAssertEqual(WeighInExplanation.text(unit: .stLb), "Weekly swings of two or three pounds are normal and mean nothing on their own.")
    }
